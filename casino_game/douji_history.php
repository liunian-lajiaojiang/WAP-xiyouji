...
$history = Database::queryAll(
    "SELECT b.*, r.hg_init_hp, r.lw_init_hp, r.winner as round_winner
     FROM douji_bets b
     LEFT JOIN douji_rounds r ON b.round_id = r.id
     WHERE b.char_id = ?
...
<head>
    ...
    <title>斗鸡房历史_<?= h(SERVER_NAME) ?></title>
...
<table border="1" style="width:100%; font-size:12px;">
    ...
    <tbody>
        <?php foreach ($history as $r):
            $winnerName = $r['round_winner'] === null ? '双败赔本' : ($kindNames[$r['round_winner']] ?? $r['round_winner']);
            $netWin = $r['is_win'] ? ($r['win_amount'] - $r['commission']) : 0;
        ?>
        <tr>
            <td><?= date('m-d H:i', strtotime($r['created_at'])) ?></td>
            <td>#<?= $r['round_id'] ?></td>
            <td style="color:<?= $r['bet_kind'] === 'hg' ? '#FF4444' : '#44BB44' ?>;">
                <?= $kindNames[$r['bet_kind']] ?? $r['bet_kind'] ?>
            </td>
